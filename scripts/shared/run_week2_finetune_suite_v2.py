"""Week-2 fine-tune suite – v2 runner.

Re-runs the fine-tuning for all (family, N, seed) combinations using the
updated finetune_last2_alignn.py script.  Results are written to a new
``finetune_last2_v2`` sub-directory inside each run folder so that the
previous ``finetune_last2`` results are preserved.

Usage (from repo root)::

    python scripts/shared/run_week2_finetune_suite_v2.py \\
...
# Sub-directory name used for v2 results (old runs kept in finetune_last2).
OUTPUT_SUBDIR = "finetune_last2_v2"
CONFIG_SUFFIX = "finetune_last2_v2"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the week-2 fine-tune suite (v2) for all families/Ns/seeds."
    )
    ...
    parser.add_argument("--force", action="store_true", help="Re-run even if summary.json already exists.")
    ...
    for family in args.families:
        for n_value in args.Ns:
            for seed in args.seeds:
                ...
                config_path = (
                    repo / args.configs_dir / f"{family}_week2_N{n_value}_seed{seed}.{CONFIG_SUFFIX}.json"
                )
    ...
    summary_out = repo / args.reports_dir / "run_suite_v2_summary.json"
