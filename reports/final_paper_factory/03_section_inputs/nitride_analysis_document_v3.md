...
- **C1. Checkpoint terminology.** The pretrained formation-energy ALIGNN model is not described as "oxide-pretrained." The JARVIS-DFT pretraining corpus includes nitrides; it is consistent with a pretraining regime more aligned with oxides than nitrides, not an oxide-exclusive one. "Oxide-reference region" is used only in distance-context language in Section 4.
...
1. **Step 1 — zero-shot gap.** The pretrained model is already about twice as inaccurate on nitrides as on oxides. The domain-shift penalty exists before any fine-tuning.
