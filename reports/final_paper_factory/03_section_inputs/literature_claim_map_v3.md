...
**Version note (v3).** Consistency patch applied after JARVIS 2020 was confirmed present in `paper_sources/`. Stale "to be obtained" language removed; JARVIS 2020 is now treated as a fully available local source candidate matching the established primary-provenance role. No citation hierarchies changed; no claims added or broadened.

**Authority note.** This file is the source-tier and claim-boundary authority for the Stage 5 planning pack. `introduction_paragraph_plan_v4.md` is the authoritative introduction blueprint. `citation_needed_list_v2.md` is the practical citation helper for Methods, Discussion, and Abstract — it must not contradict this file or the intro blueprint.
...
| JARVIS ecosystem / benchmark context | **Primary (dataset/repository provenance):** `paper_sources/Choudhary et al. - 2020 - The joint automated repository for various integrated simulations (JARVIS) for data-driven materials.pdf`. **Secondary (broader ecosystem/pretrained infrastructure):** `paper_sources/Choudhary - 2025 - The JARVIS Infrastructure is All You Need for Materials Design.pdf`. **Contextual (benchmarking/leaderboard framing only):** `paper_sources/Choudhary et al. - 2024 - JARVIS-Leaderboard a large scale benchmark of materials design methods.pdf` | JARVIS 2020 is the primary anchor for where the DFT dataset and repository come from. JARVIS 2025 covers broader pretrained-model ecosystem context. JARVIS-Leaderboard 2024 is used only when the sentence explicitly concerns benchmark infrastructure or leaderboard standing — not as a generic JARVIS citation. |
| Materials-data ecosystem background | `paper_sources/Jain et al. - 2013 - Commentary The Materials Project A materials genome approach to accelerating materials innovation.pdf`; `paper_sources/Dunn et al. - 2020 - Benchmarking materials property prediction methods the Matbench test set and Automatminer reference.pdf` | Optional background only. Include only if the intro or methods section genuinely needs broader benchmarking or data-ecosystem context. Do not force Matbench into the paper to appear scholarly. |
