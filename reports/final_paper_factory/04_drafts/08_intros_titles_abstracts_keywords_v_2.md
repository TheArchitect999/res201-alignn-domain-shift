...
2. **The In-Distribution Control Arm: Pretraining Value and Fine-Tuning Behaviour in Chemistry-Aligned Formation-Energy Prediction**

Preferred lead: **Title 1** — it cleanly signals the scope (oxide), the model family (ALIGNN), the property (formation energy), the methodological role (in-distribution control), and the measurement surface (data-efficient transfer learning) without previewing any result. Title 2 is retained as a conservative reserve option. The earlier over-strong alternatives that leaned on "ceiling," "already enough," or "saturation" wording are not carried forward in the final shortlist.
...
1. **The Domain-Shift Penalty of Pretrained ALIGNN on Nitride Formation-Energy Prediction: Behavioural and Representational Evidence**
2. **When Transfer Stalls: A Chemical-Family Domain-Shift Evaluation of Pretrained ALIGNN on Nitride Formation Energies**
3. **Inert Fine-Tuning and Embedding Geometry: Pretrained ALIGNN on a Chemistry-Distant Nitride Target**
4. **Out-of-Distribution Transfer in Crystal-Graph Models: Data-Efficiency and Representation Geometry on JARVIS Nitrides**

Preferred lead: **Title 1** — it names the mechanism under test (domain shift), the measurement surfaces (behavioural and representational), the model family (ALIGNN), and the target (nitride formation energies) without previewing a specific number. Title 2 is a more rhetorical reserve option if a stronger opening is preferred. Titles 3 and 4 remain conservative reserves. The earlier absolute title claiming that fine-tuning cannot close the chemistry-family gap is not retained.
...
- It does not adjudicate between the preferred lead title and reserve title options in any stream beyond marking the current lead choice.

---

## 6. Short changelog

- Pruned the oxide final title shortlist to remove over-strong "ceiling," "already enough," and "saturation" formulations.
- Pruned the nitride final title shortlist to remove the absolute "cannot close" formulation.
- Softened oxide Abstract A so the initialization claim is explicitly bounded to the tested range and protocol.
- Softened the nitride introduction so the embedding analysis strengthens the domain-shift reading rather than "licensing" it.
- Softened combined-paper wording by changing "isolate" to "examine ... under a matched protocol."
- Replaced combined-paper "mechanism-oriented" language with representation-space interpretive / correlate language.
- Softened combined abstract endings so they describe family-level transfer behaviour under the tested protocol rather than a general law across chemistries.
