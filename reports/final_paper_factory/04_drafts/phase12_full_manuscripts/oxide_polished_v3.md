...
Pretrained crystal-graph neural networks are a routine starting point for materials-property prediction in low-data regimes, but their data-efficiency value depends on how closely the target chemistry matches the distribution that supported pretraining. This report evaluates a single pretrained formation-energy ALIGNN model on an in-distribution oxide control task, establishing the reference condition against which the companion chemistry-shifted evaluation is read. On the JARVIS `dft_3d` benchmark splits, we combine three measurement surfaces: zero-shot evaluation of the pretrained checkpoint, fine-tuning across six labelled-data sizes from ten to one thousand structures with five random seeds per size, and matched from-scratch baselines at the two sizes where both protocols are available. We additionally extract frozen `last_alignn_pool` embeddings to describe how oxides are organized in the pretrained representation. Under the canonical Hyperparameter Set 1 protocol, the pretrained zero-shot checkpoint is already the strongest observed oxide configuration; canonical fine-tuning converges monotonically toward but does not cross that benchmark across the tested range; and pretrained initialization outperforms random initialization by a wide margin at both scratch-tested sizes. In the frozen representation, oxides occupy a cohesive, locally pure region. Scoped to the tested regime, these findings identify the choice of initialization as the dominant data-efficiency lever, support pretrained zero-shot as a defensible first-pass estimator on chemistry-aligned formation-energy targets, and fix the in-distribution reference condition needed to interpret the chemistry-shifted nitride arm as domain shift.
