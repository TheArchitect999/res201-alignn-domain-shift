...
The broader role of these findings is to fix the project's reference condition. The oxide arm is the case in which the pretrained representation operates inside the regime it was prepared for — the control without which the companion nitride report's evidence cannot be diagnosed as domain shift rather than as small-data noise or optimizer pathology. By establishing what transfer working as advertised looks like under the canonical protocol, this report renders the chemical-family domain shift documented in the companion work an interpretable, quantifiable phenomenon rather than an isolated observation.
