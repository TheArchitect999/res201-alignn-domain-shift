...
\begin{enumerate}
  ...
  \item writes both a predictions CSV and a summary JSON.
\end{enumerate}
...
python scripts/shared/zero_shot_alignn.py --family oxide --repo-root .
python scripts/shared/zero_shot_alignn.py --family nitride --repo-root .
\end{lstlisting}
...
\begin{enumerate}
  ...
  \item copy the corresponding POSCAR files into \texttt{results/<family>/N50\_seed0/dataset\_root};
  \item write \texttt{id\_prop.csv} and \texttt{split\_manifest.json}.
\end{enumerate}

Representative commands used:
\begin{lstlisting}[language=bash]
python scripts/shared/prepare_week1_finetune_dataset.py \
...
Representative commands used:
\begin{lstlisting}[language=bash]
python scripts/shared/finetune_last2_alignn.py \
  --config configs/oxide_week1_N50_seed0.finetune_last2.json \
  --output-dir results/oxide/N50_seed0/finetune_last2 \
  --dataset-root results/oxide/N50_seed0/dataset_root \
...
python scripts/shared/finetune_last2_alignn.py \
  ...
  --output-dir results/nitride/N50_seed0/finetune_last2 \
  --dataset-root results/nitride/N50_seed0/dataset_root \
  --pretrained-checkpoint jv_formation_energy_peratom_alignn/checkpoint_300.pt \
  --pretrained-config jv_formation_energy_peratom_alignn/config.json \
  --device cpu
\end{lstlisting}
...
Oxide zero-shot MAE reported & Done & \texttt{results/oxide/zero\_shot/summary.json} \\
Nitride zero-shot MAE reported & Done & \texttt{results/nitride/zero\_shot/summary.json} \\
One oxide fine-tune at $N=50$ completed & Done & \texttt{results/oxide/N50\_seed0/finetune\_last2/summary.json} \\
One nitride fine-tune at $N=50$ completed & Done & \texttt{results/nitride/N50\_seed0/finetune\_last2/summary.json} \\
\bottomrule
\end{longtable}
...
\begin{lstlisting}[language=bash]
scripts/shared/check_week1_status.sh .
\end{lstlisting}
...
python -m json.tool results/oxide/zero_shot/summary.json
python -m json.tool results/nitride/zero_shot/summary.json
python -m json.tool results/oxide/N50_seed0/finetune_last2/summary.json
python -m json.tool results/nitride/N50_seed0/finetune_last2/summary.json
...
\texttt{cache/} & Local JARVIS download/cache area. \\
...
\texttt{id\_prop.csvtrain\_data/}, \texttt{id\_prop.csvval\_data/}, \texttt{id\_prop.csvtest\_data/} & Root-level LMDB caches created by ALIGNN training utilities. These are local/generated artifacts, not canonical research outputs. \\
...
\texttt{results/} & Week 1 outputs, including zero-shot predictions and $N=50$ oxide/nitride experiment folders. \\
...
\paragraph{results}
\begin{itemize}
  \item \texttt{results/oxide/zero\_shot/}: oxide predictions CSV and summary JSON.
  \item \texttt{results/nitride/zero\_shot/}: nitride predictions CSV and summary JSON.
  \item \texttt{results/oxide/N50\_seed0/dataset\_root/}: local oxide Week 1 POSCAR subset, \texttt{id\_prop.csv}, and split manifest.
  \item \texttt{results/nitride/N50\_seed0/dataset\_root/}: local nitride Week 1 POSCAR subset, \texttt{id\_prop.csv}, and split manifest.
  \item \texttt{results/oxide/N50\_seed0/finetune\_last2/}: completed oxide Week 1 fine-tune outputs.
  \item \texttt{results/nitride/N50\_seed0/finetune\_last2/}: completed nitride Week 1 fine-tune outputs.
  \item \texttt{results/oxide/N50\_seed0/train\_alignn\_restart/}: older partial attempt from the original debugging path.
\end{itemize}
...
\begin{itemize}
  ...
  \item copied POSCARs inside \texttt{results/*/dataset\_root}.
\end{itemize}
...
git status
git add .gitignore
git add scripts/shared/check_week1_status.sh
git add scripts/shared/finetune_last2_alignn.py
...
git add results/oxide/zero_shot/summary.json
git add results/oxide/zero_shot/predictions.csv
git add results/nitride/zero_shot/summary.json
git add results/nitride/zero_shot/predictions.csv
git add results/oxide/N50_seed0/finetune_last2/*.json
git add results/oxide/N50_seed0/finetune_last2/*.csv
git add results/nitride/N50_seed0/finetune_last2/*.json
git add results/nitride/N50_seed0/finetune_last2/*.csv
