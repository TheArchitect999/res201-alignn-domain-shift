...
Final recommendation: use `data_shared/` as the frozen dataset source, `jv_formation_energy_peratom_alignn/` as the local pretrained ALIGNN source, `results/{oxide,nitride}/zero_shot/` as the canonical zero-shot prediction source, and `results_prof_advice/` with `reports/Hyperparameter Set 1/` as the canonical namespace for error-linked fine-tuned and from-scratch analysis.
...
|---|---|---|---|
| Oxide zero-shot predictions | `results/oxide/zero_shot/predictions.csv` | Join `jid`, `filename`, `target`, `prediction`, and `abs_error` to test-set structures. | Authoritative zero-shot error table. |
| Oxide zero-shot summary | `results/oxide/zero_shot/summary.json` | Zero-shot MAE and test count. Count: 1,484, MAE: 0.03418360680813096. | Authoritative summary, but contains stale historical absolute path text. |
| Nitride zero-shot predictions | `results/nitride/zero_shot/predictions.csv` | Join `jid`, `filename`, `target`, `prediction`, and `abs_error` to test-set structures. | Authoritative zero-shot error table. |
| Nitride zero-shot summary | `results/nitride/zero_shot/summary.json` | Zero-shot MAE and test count. Count: 242, MAE: 0.06954201496284854. | Authoritative summary, but contains stale historical absolute path text. |
| Report-level zero-shot summaries | `reports/Hyperparameter Set 1/Summaries/Finetuning/zero_shot_summary.csv`, `reports/Hyperparameter Set 2/Summaries/Finetuning/zero_shot_summary.csv`, `reports/Hyperparameter Set 3/Summaries/Finetuning/zero_shot_summary.csv` | Convenience copies for report bundles. | Secondary summaries, not canonical inputs. |

## Canonical Error-Linked Namespace

Use `results_prof_advice/` with `reports/Hyperparameter Set 1/`.

| Asset | Path | Use | Authority |
|---|---|---|---|
| Fine-tuning namespace | `results_prof_advice/{oxide,nitride}/N*_seed*/finetune_last2/` | Error-linked predictions for project-brief/professor-advice fine-tuned runs. | Canonical experiment namespace for fine-tuned error analysis. |
| Fine-tuning predictions | `results_prof_advice/{oxide,nitride}/N*_seed*/finetune_last2/prediction_results_test_set.csv` | Per-run `id`, `target`, and `prediction` on the fixed test set. | Authoritative per-run prediction table. |
| Fine-tuning summaries | `results_prof_advice/{oxide,nitride}/N*_seed*/finetune_last2/summary.json` | Per-run MAE and metadata. | Authoritative per-run summary. |
| Copied run structure roots | `results_prof_advice/{oxide,nitride}/N*_seed*/dataset_root/` | Per-run structure copies and split manifests for direct error-to-structure linking. | Authoritative run-level provenance. |
...
| From-scratch namespace | `results_prof_advice/{oxide,nitride}/N*_seed*/train_alignn_fromscratch/` | Error-linked predictions for project-brief/professor-advice from-scratch baselines. | Canonical baseline namespace for comparison. |
| From-scratch predictions | `results_prof_advice/{oxide,nitride}/N*_seed*/train_alignn_fromscratch/prediction_results_test_set.csv` | Per-run test predictions for baseline error comparison. | Authoritative per-run prediction table. |
...
Completeness checked read-only: `results_prof_advice/` has 60 fine-tuning summaries and 20 from-scratch summaries, with aggregate report artifacts present.

## Alternative Namespaces

| Namespace | Report bundle | Status | Role |
|---|---|---|---|
| `results_prof_advice_alignn_recommended/` | `reports/Hyperparameter Set 2/` | Complete: 60 fine-tuning runs and 20 from-scratch runs. | Authoritative comparison namespace for ALIGNN-recommended hyperparameters, not the canonical error-linked namespace for this phase. |
| `results/` | `reports/Hyperparameter Set 3/` for Set 3 extension, plus `results/{oxide,nitride}/zero_shot/` | Complete Set 3 extension and canonical zero-shot outputs. | Canonical for zero-shot; Set 3 is an additional low-learning-rate comparison. |
...
- The local zero-shot `summary.json` files contain historical absolute paths in metadata. The repo-relative prediction CSVs under `results/{oxide,nitride}/zero_shot/` are the canonical files to use.
...
- Zero-shot errors: `results/oxide/zero_shot/predictions.csv` and `results/nitride/zero_shot/predictions.csv`.
- Fine-tuned and from-scratch error-linked namespace: `results_prof_advice/`.
